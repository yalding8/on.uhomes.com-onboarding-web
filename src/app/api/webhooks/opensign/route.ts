...
export async function POST(request: Request) {
  try {
    ...
    if (!signatureHeader || signatureHeader !== process.env.OPENSIGN_WEBHOOK_SECRET) {
    ...
    if (!request_id || event_type !== 'signature_request_signed') {
      return NextResponse.json({ message: 'Irrelevant notification ignored' }, { status: 200 })
    }
    ...
    if (findContractError || !contract) {
      console.error('Webhook error: Contract record not found for webhook request ID:', request_id)
      return NextResponse.json({ error: 'Contract link broken or not found' }, { status: 404 })
    }
    ...
    if (updateContractError) {
      throw new Error(`Failed to update contract: ${updateContractError.message}`)
    }
    ...
    if (updateSupplierError) {
      throw new Error(`Failed to update supplier state: ${updateSupplierError.message}`)
    }
  
    // TODO Phase 4.1: Parallel launch data aggregation pipeline for auto-onboarding listing.

    return NextResponse.json({ success: true, processed_contract_id: contract.id })

  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown server error processing webhook'
